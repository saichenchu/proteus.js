...
describe 'IdentityKeyPair', ->
  it 'serialises and deserialises', ->
    ...
    assert(ikp.public_key.fingerprint() is ikp_deser.public_key.fingerprint())
    assert(sodium.to_hex(new Uint8Array ikp_bytes) is sodium.to_hex(new Uint8Array ikp_deser.serialise()))
