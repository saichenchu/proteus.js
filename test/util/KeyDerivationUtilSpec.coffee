...
run_hkdf_testcase = (v) ->
  it v.case, ->
    ...
    result = sodium.to_hex Proteus.util.KeyDerivationUtil.hkdf a..., v.length

    assert(result is v.okm)
    assert(result.length is (v.length * 2))
...
describe 'HMAC Real World Scenarios', ->
  vectors = [
      ...
      salt:   undefined
      length: 64
      okm:    '16390fa0e7e7a9b7cd16b3e435330e18a02df7136a154e5c6e3818549c75b1b0baa38ee1d38364a45033b42ab18448faf6f1296c928cf7c5cdf22096ba7a2d39'
    ,
  ]
